...
<template>
	...
	<v-layout row>
		...
		<v-navigation-drawer permanent app clipped value="true">
			<v-toolbar flat>Mes calepins</v-toolbar>
    	<v-divider></v-divider>
			<v-subheader>Coterie personnelle</v-subheader>
      <v-list v-for="(coterie, index) in [coterie_perso]">
				<v-list-tile :key="index" @click="switchCoterie(coterie); refreshPad();" v-model="coterie_courante.id === coterie.id">
